...
contract MyDevelopBank{
    //mapping for to map address with balance
    ...
    function depositeAmount() payable external {
        require(msg.value>0,"You can not add Zero value");
    ...
    function toTransfer(address receiver,uint amount) external  Checker(amount) {
        require(amount>0,"You can not send Zero value");
